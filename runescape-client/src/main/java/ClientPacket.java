import net.runelite.mapping.Export;
import net.runelite.mapping.Implements;
import net.runelite.mapping.ObfuscatedGetter;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("mm")
@Implements("ClientPacket")
public class ClientPacket implements class321 {
	@ObfuscatedName("ay")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPCU;
	@ObfuscatedName("ah")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket FREECAM_EXIT;
	@ObfuscatedName("az")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ3;
	@ObfuscatedName("ao")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket LOGIN_TIMINGS;
	@ObfuscatedName("ad")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MOUSE_MOVE;
	@ObfuscatedName("an")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER4;
	@ObfuscatedName("ae")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPCT;
	@ObfuscatedName("ax")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON1;
	@ObfuscatedName("at")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket DETECT_MODIFIED_CLIENT;
	@ObfuscatedName("ac")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER7;
	@ObfuscatedName("au")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON7;
	@ObfuscatedName("as")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON9;
	@ObfuscatedName("aq")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket REFLECTION_CHECK_REPLY;
	@ObfuscatedName("af")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON8;
	@ObfuscatedName("aa")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket FRIEND_ADDUSER;
	@ObfuscatedName("ak")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON6;
	@ObfuscatedName("al")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CHAT_SETFILTER;
	@ObfuscatedName("av")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOCE;
	@ObfuscatedName("ag")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket FRIEND_CHAT_JOIN_LEAVE;
	@ObfuscatedName("aw")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_PAUSEBUTTON;
	@ObfuscatedName("ai")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MINIMAP_CLICK;
	@ObfuscatedName("ar")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CLAN_CHANNEL_KICK_USER;
	@ObfuscatedName("am")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON5;
	@ObfuscatedName("ap")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER3;
	@ObfuscatedName("ab")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CLOSE_MODAL;
	@ObfuscatedName("aj")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYERU;
	@ObfuscatedName("bu")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ1;
	@ObfuscatedName("bh")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ2;
	@ObfuscatedName("bw")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket DOCHEAT;
	@ObfuscatedName("bb")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ4;
	@ObfuscatedName("bx")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON4;
	@ObfuscatedName("bd")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC4;
	@ObfuscatedName("bi")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3371;
	@ObfuscatedName("bz")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON2;
	@ObfuscatedName("bt")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CLAN_KICKUSER;
	@ObfuscatedName("bf")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3409;
	@ObfuscatedName("by")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CLICK_WORLD_MAP;
	@ObfuscatedName("bm")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER6;
	@ObfuscatedName("bp")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_KEYBOARD;
	@ObfuscatedName("bg")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER8;
	@ObfuscatedName("bc")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC5;
	@ObfuscatedName("bv")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_COUNTDIALOG;
	@ObfuscatedName("bq")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC1;
	@ObfuscatedName("ba")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_NAMEDIALOG;
	@ObfuscatedName("bk")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC4;
	@ObfuscatedName("bs")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3342;
	@ObfuscatedName("bj")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_APPLET_FOCUS;
	@ObfuscatedName("bo")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC1;
	@ObfuscatedName("be")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket UPDATE_PLAYER_MODEL;
	@ObfuscatedName("bn")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER5;
	@ObfuscatedName("br")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_WINDOW_SETTING;
	@ObfuscatedName("bl")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CHAT_SENDPRIVATE;
	@ObfuscatedName("cb")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket TELEPORT;
	@ObfuscatedName("cn")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket WIDGET_TYPE;
	@ObfuscatedName("cc")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON10;
	@ObfuscatedName("co")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC3;
	@ObfuscatedName("ca")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket PING_STATISTICS;
	@ObfuscatedName("cg")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYERT;
	@ObfuscatedName("cp")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket field3374;
	@ObfuscatedName("cz")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPHELDD;
	@ObfuscatedName("cy")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTON3;
	@ObfuscatedName("cu")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJE;
	@ObfuscatedName("cq")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MOVE_GAMECLICK;
	@ObfuscatedName("cf")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket FRIEND_CHAT_SETRANK;
	@ObfuscatedName("ct")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER1;
	@ObfuscatedName("ci")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC5;
	@ObfuscatedName("cl")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC2;
	@ObfuscatedName("cw")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPCE;
	@ObfuscatedName("cj")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket BUG_REPORT;
	@ObfuscatedName("cx")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket SET_HEADING;
	@ObfuscatedName("cd")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CHAT_SENDABUSEREPORT;
	@ObfuscatedName("cv")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJU;
	@ObfuscatedName("ck")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MOUSE_WHEEL;
	@ObfuscatedName("cs")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_CAMERA_POSITION;
	@ObfuscatedName("cm")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_CRMVIEW;
	@ObfuscatedName("ce")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJ5;
	@ObfuscatedName("cr")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3392;
	@ObfuscatedName("ch")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOCU;
	@ObfuscatedName("dh")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOC2;
	@ObfuscatedName("dn")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3395;
	@ObfuscatedName("dc")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MESSAGE_PUBLIC;
	@ObfuscatedName("dm")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPNPC3;
	@ObfuscatedName("dj")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MAP_BUILD_COMPLETE;
	@ObfuscatedName("dk")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IGNORE_DELUSER;
	@ObfuscatedName("ds")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_OBJDIALOG;
	@ObfuscatedName("dr")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPPLAYER2;
	@ObfuscatedName("dl")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPOBJT;
	@ObfuscatedName("do")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket FRIEND_DELUSER;
	@ObfuscatedName("dg")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket OPLOCT;
	@ObfuscatedName("dt")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket NO_TIMEOUT;
	@ObfuscatedName("df")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket RESUME_STRINGDIALOG;
	@ObfuscatedName("de")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CLAN_SETTINGS_ADD_BANNED_FROM_CHANNEL;
	@ObfuscatedName("dw")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_MOUSE_IDLE;
	@ObfuscatedName("dv")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket MUSIC_PLAYING;
	@ObfuscatedName("dx")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3410;
	@ObfuscatedName("dd")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket CLAN_SETTINGS_SET_MUTED_FROM_CHANNEL;
	@ObfuscatedName("dy")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_BUTTONT;
	@ObfuscatedName("db")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket IF_SUBOP;
	@ObfuscatedName("du")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	public static final ClientPacket EVENT_MOUSE_CLICK;
	@ObfuscatedName("da")
	@ObfuscatedSignature(
		descriptor = "Lmm;"
	)
	static final ClientPacket field3415;
	@ObfuscatedName("lu")
	@ObfuscatedGetter(
		intValue = -1846480423
	)
	@Export("oculusOrbFocalPointZ")
	static int oculusOrbFocalPointZ;
	@ObfuscatedName("dp")
	@ObfuscatedGetter(
		intValue = -188284339
	)
	@Export("id")
	final int id;
	@ObfuscatedName("di")
	@ObfuscatedGetter(
		intValue = -1544080649
	)
	@Export("length")
	final int length;

	static {
		OPNPCU = new ClientPacket(0, 11);
		FREECAM_EXIT = new ClientPacket(1, 0);
		OPOBJ3 = new ClientPacket(2, 7);
		LOGIN_TIMINGS = new ClientPacket(3, -1);
		MOUSE_MOVE = new ClientPacket(4, -1);
		OPPLAYER4 = new ClientPacket(5, 3);
		OPNPCT = new ClientPacket(6, 11);
		IF_BUTTON1 = new ClientPacket(7, 8);
		DETECT_MODIFIED_CLIENT = new ClientPacket(8, 4);
		OPPLAYER7 = new ClientPacket(9, 3);
		IF_BUTTON7 = new ClientPacket(10, 8);
		IF_BUTTON9 = new ClientPacket(11, 8);
		REFLECTION_CHECK_REPLY = new ClientPacket(12, -1);
		IF_BUTTON8 = new ClientPacket(13, 8);
		FRIEND_ADDUSER = new ClientPacket(14, -1);
		IF_BUTTON6 = new ClientPacket(15, 8);
		CHAT_SETFILTER = new ClientPacket(16, 3);
		OPLOCE = new ClientPacket(17, 2);
		FRIEND_CHAT_JOIN_LEAVE = new ClientPacket(18, -1);
		RESUME_PAUSEBUTTON = new ClientPacket(19, 6);
		MINIMAP_CLICK = new ClientPacket(20, -1);
		CLAN_CHANNEL_KICK_USER = new ClientPacket(21, -1);
		IF_BUTTON5 = new ClientPacket(22, 8);
		OPPLAYER3 = new ClientPacket(23, 3);
		CLOSE_MODAL = new ClientPacket(24, 0);
		OPPLAYERU = new ClientPacket(25, 11);
		OPOBJ1 = new ClientPacket(26, 7);
		OPOBJ2 = new ClientPacket(27, 7);
		DOCHEAT = new ClientPacket(28, -1);
		OPOBJ4 = new ClientPacket(29, 7);
		IF_BUTTON4 = new ClientPacket(30, 8);
		OPNPC4 = new ClientPacket(31, 3);
		field3371 = new ClientPacket(32, 1);
		IF_BUTTON2 = new ClientPacket(33, 8);
		CLAN_KICKUSER = new ClientPacket(34, -1);
		field3409 = new ClientPacket(35, -1);
		CLICK_WORLD_MAP = new ClientPacket(36, 4);
		OPPLAYER6 = new ClientPacket(37, 3);
		EVENT_KEYBOARD = new ClientPacket(38, -2);
		OPPLAYER8 = new ClientPacket(39, 3);
		OPLOC5 = new ClientPacket(40, 7);
		RESUME_COUNTDIALOG = new ClientPacket(41, 4);
		OPNPC1 = new ClientPacket(42, 3);
		RESUME_NAMEDIALOG = new ClientPacket(43, -1);
		OPLOC4 = new ClientPacket(44, 7);
		field3342 = new ClientPacket(45, 1);
		EVENT_APPLET_FOCUS = new ClientPacket(46, 1);
		OPLOC1 = new ClientPacket(47, 7);
		UPDATE_PLAYER_MODEL = new ClientPacket(48, 26);
		OPPLAYER5 = new ClientPacket(49, 3);
		EVENT_WINDOW_SETTING = new ClientPacket(50, 5);
		CHAT_SENDPRIVATE = new ClientPacket(51, -2);
		TELEPORT = new ClientPacket(52, 9);
		WIDGET_TYPE = new ClientPacket(53, 4);
		IF_BUTTON10 = new ClientPacket(54, 8);
		OPLOC3 = new ClientPacket(55, 7);
		PING_STATISTICS = new ClientPacket(56, 10);
		OPPLAYERT = new ClientPacket(57, 11);
		field3374 = new ClientPacket(58, -1);
		OPHELDD = new ClientPacket(59, 16);
		IF_BUTTON3 = new ClientPacket(60, 8);
		OPOBJE = new ClientPacket(61, 6);
		MOVE_GAMECLICK = new ClientPacket(62, -1);
		FRIEND_CHAT_SETRANK = new ClientPacket(63, -1);
		OPPLAYER1 = new ClientPacket(64, 3);
		OPNPC5 = new ClientPacket(65, 3);
		OPNPC2 = new ClientPacket(66, 3);
		OPNPCE = new ClientPacket(67, 2);
		BUG_REPORT = new ClientPacket(68, -2);
		SET_HEADING = new ClientPacket(69, 1);
		CHAT_SENDABUSEREPORT = new ClientPacket(70, -1);
		OPOBJU = new ClientPacket(71, 15);
		MOUSE_WHEEL = new ClientPacket(72, 2);
		EVENT_CAMERA_POSITION = new ClientPacket(73, 4);
		IF_CRMVIEW = new ClientPacket(74, 22);
		OPOBJ5 = new ClientPacket(75, 7);
		field3392 = new ClientPacket(76, -1);
		OPLOCU = new ClientPacket(77, 15);
		OPLOC2 = new ClientPacket(78, 7);
		field3395 = new ClientPacket(79, 7);
		MESSAGE_PUBLIC = new ClientPacket(80, -1);
		OPNPC3 = new ClientPacket(81, 3);
		MAP_BUILD_COMPLETE = new ClientPacket(82, 0);
		IGNORE_DELUSER = new ClientPacket(83, -1);
		RESUME_OBJDIALOG = new ClientPacket(84, 2);
		OPPLAYER2 = new ClientPacket(85, 3);
		OPOBJT = new ClientPacket(86, 15);
		FRIEND_DELUSER = new ClientPacket(87, -1);
		OPLOCT = new ClientPacket(88, 15);
		NO_TIMEOUT = new ClientPacket(89, 0);
		RESUME_STRINGDIALOG = new ClientPacket(90, -1);
		CLAN_SETTINGS_ADD_BANNED_FROM_CHANNEL = new ClientPacket(91, -1);
		EVENT_MOUSE_IDLE = new ClientPacket(92, 0);
		MUSIC_PLAYING = new ClientPacket(93, 4);
		field3410 = new ClientPacket(94, 2);
		CLAN_SETTINGS_SET_MUTED_FROM_CHANNEL = new ClientPacket(95, -1);
		IF_BUTTONT = new ClientPacket(96, 16);
		IF_SUBOP = new ClientPacket(97, 10);
		EVENT_MOUSE_CLICK = new ClientPacket(98, 6);
		field3415 = new ClientPacket(99, 1);
	}

	ClientPacket(int var1, int var2) {
		this.id = var1;
		this.length = var2;
	}

	@ObfuscatedName("ad")
	@ObfuscatedSignature(
		descriptor = "(IIIZIZI)V",
		garbageValue = "1013551658"
	)
	@Export("doWorldSorting")
	static void doWorldSorting(int var0, int var1, int var2, boolean var3, int var4, boolean var5) {
		if (var0 < var1) {
			int var6 = (var0 + var1) / 2;
			int var7 = var0;
			World var8 = World.World_worlds[var6];
			World.World_worlds[var6] = World.World_worlds[var1];
			World.World_worlds[var1] = var8;

			for (int var9 = var0; var9 < var1; ++var9) {
				if (MouseHandler.method703(World.World_worlds[var9], var8, var2, var3, var4, var5) <= 0) {
					World var10 = World.World_worlds[var9];
					World.World_worlds[var9] = World.World_worlds[var7];
					World.World_worlds[var7++] = var10;
				}
			}

			World.World_worlds[var1] = World.World_worlds[var7];
			World.World_worlds[var7] = var8;
			doWorldSorting(var0, var7 - 1, var2, var3, var4, var5);
			doWorldSorting(var7 + 1, var1, var2, var3, var4, var5);
		}

	}
}

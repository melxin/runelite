import net.runelite.mapping.Export;
import net.runelite.mapping.Implements;
import net.runelite.mapping.ObfuscatedGetter;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("mu")
@Implements("ClientPacket")
public class ClientPacket implements class323 {
	@ObfuscatedName("ab")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPNPCU;
	@ObfuscatedName("ay")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPOBJU;
	@ObfuscatedName("au")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYERT;
	@ObfuscatedName("ad")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket RESUME_OBJDIALOG;
	@ObfuscatedName("ah")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPLOC1;
	@ObfuscatedName("ac")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON1;
	@ObfuscatedName("ao")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket PING_STATISTICS;
	@ObfuscatedName("af")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPOBJ1;
	@ObfuscatedName("aa")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket EVENT_CAMERA_POSITION;
	@ObfuscatedName("as")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CLAN_CHANNEL_KICK_USER;
	@ObfuscatedName("ae")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPOBJ3;
	@ObfuscatedName("ax")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CHAT_SENDPRIVATE;
	@ObfuscatedName("ai")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket REFLECTION_CHECK_REPLY;
	@ObfuscatedName("ak")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket FRIEND_CHAT_JOIN_LEAVE;
	@ObfuscatedName("aj")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPLOCT;
	@ObfuscatedName("av")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPNPC1;
	@ObfuscatedName("ar")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket WIDGET_TYPE;
	@ObfuscatedName("ag")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket TELEPORT;
	@ObfuscatedName("az")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	static final ClientPacket field3398;
	@ObfuscatedName("am")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYER4;
	@ObfuscatedName("aq")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CLOSE_MODAL;
	@ObfuscatedName("aw")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	static final ClientPacket field3399;
	@ObfuscatedName("an")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket FRIEND_ADDUSER;
	@ObfuscatedName("at")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYER3;
	@ObfuscatedName("ap")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPNPC2;
	@ObfuscatedName("al")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON5;
	@ObfuscatedName("bw")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket RESUME_STRINGDIALOG;
	@ObfuscatedName("bo")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	static final ClientPacket field3407;
	@ObfuscatedName("bs")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON2;
	@ObfuscatedName("bz")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPOBJ4;
	@ObfuscatedName("bt")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket RESUME_COUNTDIALOG;
	@ObfuscatedName("ba")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket DETECT_MODIFIED_CLIENT;
	@ObfuscatedName("bg")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPLOC4;
	@ObfuscatedName("bv")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CHAT_SENDABUSEREPORT;
	@ObfuscatedName("bk")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CLAN_SETTINGS_FULL_REQUEST;
	@ObfuscatedName("bu")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket UPDATE_PLAYER_MODEL;
	@ObfuscatedName("by")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPNPC5;
	@ObfuscatedName("bh")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON3;
	@ObfuscatedName("be")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYER7;
	@ObfuscatedName("bj")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket RESUME_NAMEDIALOG;
	@ObfuscatedName("bq")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPLOC5;
	@ObfuscatedName("bx")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPHELDD;
	@ObfuscatedName("bf")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPLOC3;
	@ObfuscatedName("bc")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_SUBOP;
	@ObfuscatedName("bb")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON9;
	@ObfuscatedName("bi")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYER5;
	@ObfuscatedName("bn")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket FREECAM_EXIT;
	@ObfuscatedName("bl")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_CRMVIEW;
	@ObfuscatedName("bd")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON4;
	@ObfuscatedName("br")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket DOCHEAT;
	@ObfuscatedName("bm")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPLOCU;
	@ObfuscatedName("bp")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYER8;
	@ObfuscatedName("ch")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYER2;
	@ObfuscatedName("cd")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IGNORE_DELUSER;
	@ObfuscatedName("cp")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket LOGIN_TIMINGS;
	@ObfuscatedName("ca")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket EVENT_MOUSE_IDLE;
	@ObfuscatedName("cs")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket FRIEND_CHAT_SETRANK;
	@ObfuscatedName("cj")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYER1;
	@ObfuscatedName("cy")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket RESUME_PAUSEBUTTON;
	@ObfuscatedName("cb")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CHAT_SETFILTER;
	@ObfuscatedName("co")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPOBJ2;
	@ObfuscatedName("ct")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON10;
	@ObfuscatedName("cv")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket EVENT_KEYBOARD;
	@ObfuscatedName("cf")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CLAN_KICKUSER;
	@ObfuscatedName("ck")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CLICK_WORLD_MAP;
	@ObfuscatedName("cx")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPNPC4;
	@ObfuscatedName("cu")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPNPCT;
	@ObfuscatedName("cg")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket MOVE_GAMECLICK;
	@ObfuscatedName("cw")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPLOCE;
	@ObfuscatedName("ce")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON8;
	@ObfuscatedName("cn")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPOBJ5;
	@ObfuscatedName("ci")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket MINIMAP_CLICK;
	@ObfuscatedName("cl")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPOBJT;
	@ObfuscatedName("cc")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPNPCE;
	@ObfuscatedName("cq")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPOBJE;
	@ObfuscatedName("cz")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPNPC3;
	@ObfuscatedName("cm")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	static final ClientPacket field3456;
	@ObfuscatedName("cr")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket EVENT_MOUSE_CLICK;
	@ObfuscatedName("dc")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket MAP_BUILD_COMPLETE;
	@ObfuscatedName("dd")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket MESSAGE_PUBLIC;
	@ObfuscatedName("dy")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket EVENT_APPLET_FOCUS;
	@ObfuscatedName("dj")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket FRIEND_DELUSER;
	@ObfuscatedName("dr")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket EVENT_WINDOW_SETTING;
	@ObfuscatedName("dq")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CLAN_SETTINGS_ADD_BANNED_FROM_CHANNEL;
	@ObfuscatedName("ds")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTONT;
	@ObfuscatedName("dk")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CLAN_SETTINGS_SET_MUTED_FROM_CHANNEL;
	@ObfuscatedName("du")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPLOC2;
	@ObfuscatedName("dl")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON7;
	@ObfuscatedName("dn")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYERU;
	@ObfuscatedName("dh")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket BUG_REPORT;
	@ObfuscatedName("di")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket MOUSE_WHEEL;
	@ObfuscatedName("dw")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket MOUSE_MOVE;
	@ObfuscatedName("db")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket SET_HEADING;
	@ObfuscatedName("dt")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket OPPLAYER6;
	@ObfuscatedName("dm")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket CLAN_CHANNEL_FULL_REQUEST;
	@ObfuscatedName("do")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IGNORE_LIST_ADD;
	@ObfuscatedName("dx")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket NO_TIMEOUT;
	@ObfuscatedName("dv")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket IF_BUTTON6;
	@ObfuscatedName("df")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	public static final ClientPacket MUSIC_PLAYING;
	@ObfuscatedName("de")
	@ObfuscatedSignature(
		descriptor = "Lmu;"
	)
	static final ClientPacket field3472;
	@ObfuscatedName("da")
	@ObfuscatedGetter(
		intValue = 852466269
	)
	@Export("id")
	final int id;
	@ObfuscatedName("dz")
	@ObfuscatedGetter(
		intValue = -604126461
	)
	@Export("length")
	final int length;

	static {
		OPNPCU = new ClientPacket(0, 11);
		OPOBJU = new ClientPacket(1, 15);
		OPPLAYERT = new ClientPacket(2, 11);
		RESUME_OBJDIALOG = new ClientPacket(3, 2);
		OPLOC1 = new ClientPacket(4, 7);
		IF_BUTTON1 = new ClientPacket(5, 8);
		PING_STATISTICS = new ClientPacket(6, 10);
		OPOBJ1 = new ClientPacket(7, 7);
		EVENT_CAMERA_POSITION = new ClientPacket(8, 4);
		CLAN_CHANNEL_KICK_USER = new ClientPacket(9, -1);
		OPOBJ3 = new ClientPacket(10, 7);
		CHAT_SENDPRIVATE = new ClientPacket(11, -2);
		REFLECTION_CHECK_REPLY = new ClientPacket(12, -1);
		FRIEND_CHAT_JOIN_LEAVE = new ClientPacket(13, -1);
		OPLOCT = new ClientPacket(14, 15);
		OPNPC1 = new ClientPacket(15, 3);
		WIDGET_TYPE = new ClientPacket(16, 4);
		TELEPORT = new ClientPacket(17, 9);
		field3398 = new ClientPacket(18, 2);
		OPPLAYER4 = new ClientPacket(19, 3);
		CLOSE_MODAL = new ClientPacket(20, 0);
		field3399 = new ClientPacket(21, -1);
		FRIEND_ADDUSER = new ClientPacket(22, -1);
		OPPLAYER3 = new ClientPacket(23, 3);
		OPNPC2 = new ClientPacket(24, 3);
		IF_BUTTON5 = new ClientPacket(25, 8);
		RESUME_STRINGDIALOG = new ClientPacket(26, -1);
		field3407 = new ClientPacket(27, -1);
		IF_BUTTON2 = new ClientPacket(28, 8);
		OPOBJ4 = new ClientPacket(29, 7);
		RESUME_COUNTDIALOG = new ClientPacket(30, 4);
		DETECT_MODIFIED_CLIENT = new ClientPacket(31, 4);
		OPLOC4 = new ClientPacket(32, 7);
		CHAT_SENDABUSEREPORT = new ClientPacket(33, -1);
		CLAN_SETTINGS_FULL_REQUEST = new ClientPacket(34, 1);
		UPDATE_PLAYER_MODEL = new ClientPacket(35, 26);
		OPNPC5 = new ClientPacket(36, 3);
		IF_BUTTON3 = new ClientPacket(37, 8);
		OPPLAYER7 = new ClientPacket(38, 3);
		RESUME_NAMEDIALOG = new ClientPacket(39, -1);
		OPLOC5 = new ClientPacket(40, 7);
		OPHELDD = new ClientPacket(41, 16);
		OPLOC3 = new ClientPacket(42, 7);
		IF_SUBOP = new ClientPacket(43, 10);
		IF_BUTTON9 = new ClientPacket(44, 8);
		OPPLAYER5 = new ClientPacket(45, 3);
		FREECAM_EXIT = new ClientPacket(46, 0);
		IF_CRMVIEW = new ClientPacket(47, 22);
		IF_BUTTON4 = new ClientPacket(48, 8);
		DOCHEAT = new ClientPacket(49, -1);
		OPLOCU = new ClientPacket(50, 15);
		OPPLAYER8 = new ClientPacket(51, 3);
		OPPLAYER2 = new ClientPacket(52, 3);
		IGNORE_DELUSER = new ClientPacket(53, -1);
		LOGIN_TIMINGS = new ClientPacket(54, -1);
		EVENT_MOUSE_IDLE = new ClientPacket(55, 0);
		FRIEND_CHAT_SETRANK = new ClientPacket(56, -1);
		OPPLAYER1 = new ClientPacket(57, 3);
		RESUME_PAUSEBUTTON = new ClientPacket(58, 6);
		CHAT_SETFILTER = new ClientPacket(59, 3);
		OPOBJ2 = new ClientPacket(60, 7);
		IF_BUTTON10 = new ClientPacket(61, 8);
		EVENT_KEYBOARD = new ClientPacket(62, -2);
		CLAN_KICKUSER = new ClientPacket(63, -1);
		CLICK_WORLD_MAP = new ClientPacket(64, 4);
		OPNPC4 = new ClientPacket(65, 3);
		OPNPCT = new ClientPacket(66, 11);
		MOVE_GAMECLICK = new ClientPacket(67, -1);
		OPLOCE = new ClientPacket(68, 2);
		IF_BUTTON8 = new ClientPacket(69, 8);
		OPOBJ5 = new ClientPacket(70, 7);
		MINIMAP_CLICK = new ClientPacket(71, -1);
		OPOBJT = new ClientPacket(72, 15);
		OPNPCE = new ClientPacket(73, 2);
		OPOBJE = new ClientPacket(74, 6);
		OPNPC3 = new ClientPacket(75, 3);
		field3456 = new ClientPacket(76, 7);
		EVENT_MOUSE_CLICK = new ClientPacket(77, 6);
		MAP_BUILD_COMPLETE = new ClientPacket(78, 0);
		MESSAGE_PUBLIC = new ClientPacket(79, -1);
		EVENT_APPLET_FOCUS = new ClientPacket(80, 1);
		FRIEND_DELUSER = new ClientPacket(81, -1);
		EVENT_WINDOW_SETTING = new ClientPacket(82, 5);
		CLAN_SETTINGS_ADD_BANNED_FROM_CHANNEL = new ClientPacket(83, -1);
		IF_BUTTONT = new ClientPacket(84, 16);
		CLAN_SETTINGS_SET_MUTED_FROM_CHANNEL = new ClientPacket(85, -1);
		OPLOC2 = new ClientPacket(86, 7);
		IF_BUTTON7 = new ClientPacket(87, 8);
		OPPLAYERU = new ClientPacket(88, 11);
		BUG_REPORT = new ClientPacket(89, -2);
		MOUSE_WHEEL = new ClientPacket(90, 2);
		MOUSE_MOVE = new ClientPacket(91, -1);
		SET_HEADING = new ClientPacket(92, 1);
		OPPLAYER6 = new ClientPacket(93, 3);
		CLAN_CHANNEL_FULL_REQUEST = new ClientPacket(94, 1);
		IGNORE_LIST_ADD = new ClientPacket(95, -1);
		NO_TIMEOUT = new ClientPacket(96, 0);
		IF_BUTTON6 = new ClientPacket(97, 8);
		MUSIC_PLAYING = new ClientPacket(98, 4);
		field3472 = new ClientPacket(99, 1);
	}

	ClientPacket(int var1, int var2) {
		this.id = var1;
		this.length = var2;
	}
}
